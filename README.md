...
插件会为每次 Agent 运行记录 trace：工具调用参数摘要、结果摘要、已用/剩余工具次数、是否调用过 `qa_upsert`、最终 JSON、以及“Agent 调查期间群里新消息是否已经解决问题”的复核结果。管理员可以用 `/ph trace` 查看最近一次摘要和 trace 文件路径。

如果模型没有按最终 JSON 协议输出，插件会额外尝试一次严格 JSON 格式修复；仍然失败时会静默不回复，只写 trace，不会把原始的泛泛回答直接发到群里。
...
这些命令都需要 AstrBot 管理员权限。`/ph status` 会显示当前群绑定、仓库路径、QA 路径、trace 路径和关键运行参数；`/ph update` 会立即 clone 或更新当前群绑定的仓库；`/ph trace` 会查看最近一次 Agent 运行摘要。

Agent 运行期间如果群里又来了新消息，插件不会取消正在调查的 Agent，而是先把新消息放进运行中队列。Agent 给出答案后，插件会用这些新消息做一次轻量复核：如果群友已经把问题解决了、提问者说不用了，机器人就不再发言；如果新消息只是补充日志或新的追问，会重新进入普通缓冲，继续按 `buffer_seconds` 聚合。
